 ...
 try {
  ...
  return (
    ...
    {currentState === 'Login' ? '' : <input onChange={(e)=>setName(e.target.value)} value ={name} className='w-full px-3 py-2 border border-gray-800' type="text"  placeholder='Name' required/>}
     ...
     <button  className = 'bg-black text-white font-light px-8 py-2 mt-4 rounded-md cursor-pointer transition-transform duration-300 hover:scale-110 '>{currentState === 'Login' ? 'Sign In' : 'Sign Up'}</button>
    </form>
  )
}
